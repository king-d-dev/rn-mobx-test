 ...
 */
export async function save(username: string, password: string, server?: string): Promise<false | ReactNativeKeychain.Result> {
  if (server) {
    return await ReactNativeKeychain.setInternetCredentials(server, username, password)
  } else {
    return await ReactNativeKeychain.setGenericPassword(username, password)
  }
}
